...
// 1. Extraer el string de usuario controlando estructuras complejas de datos
$userData = $_SESSION["user"];
$rawName = "Usuario";

if (is_array($userData)) {
    $rawName = $userData['nombre'] ?? $userData['email'] ?? 'Usuario';
    ...
    <div class="flex-1 flex flex-col h-full overflow-y-auto">
        <header class="h-20 border-b border-slate-200 bg-white/80 backdrop-blur-md px-8 flex items-center justify-between sticky top-0 z-10 shrink-0">
            <div class="flex flex-col">
                ...
                <p class="text-xs text-slate-500 font-medium">Sistema moderno, inclusivo y adaptable para Instituciones Educativas.</p>
            </div>
            <div class="flex items-center gap-4">
                <div class="text-right hidden md:block">
                    <p class="text-sm font-bold text-slate-900 leading-none mb-1"><?= htmlspecialchars($userDisplay, ENT_QUOTES, 'UTF-8'); ?></p>
                ...
                <div class="w-11 h-11 rounded-xl bg-gradient-to-tr from-slate-800 to-slate-950 flex items-center justify-center text-white font-bold text-lg shadow-md border-2 border-white">
                    <?= strtoupper(substr(htmlspecialchars($userDisplay, ENT_QUOTES, 'UTF-8'), 0, 1)); ?>
                </div>
            </div>
        </header>
        ...
        <main class="p-8 space-y-8 max-w-7xl w-full mx-auto">
            ...
            <section>
                <h3 class="heading-font text-lg font-bold text-slate-900 mb-5 flex items-center gap-2">
                    <i class="fa-solid fa-rocket text-emerald-500"></i> Acciones del Ecosistema Unificado
                </h3>
